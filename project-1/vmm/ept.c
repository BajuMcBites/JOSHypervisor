...
static int ept_lookup_gpa(epte_t* eptrt, void *gpa,
	...
	epte_t* out = ept_walk_1((pml4e_t*)eptrt, gpa, create, __EPTE_FULL);
 
	if((int64_t) out < 0) return (int64_t) out;

	if (epte_out != NULL) *epte_out = out;

	return 0;
}

//walks the first level of the page table adapted from pml4e_walk
epte_t *
ept_walk_1(epte_t *dir, const void *va, int create, uint64_t perm)
{
#line 626 "../kern/pmap.c"
	if (dir) {
		pdpe_t *pdpe  = (pdpe_t *)dir [PML4(va)];
		if (!((physaddr_t)pdpe & PTE_P) && create) {
			struct PageInfo *page   = NULL;
			if ((page = page_alloc(ALLOC_ZERO))) {
				page->pp_ref += 1;
				dir [PML4(va)] = page2pa(page)|perm;
				pte_t *pte= ept_walk_2(KADDR((uintptr_t)((pdpe_t *)(PTE_ADDR(dir [PML4(va)])))),va,create, perm);
				if ((int64_t) pte < 0) return pte;
				else{
					dir[PML4(va)] = 0;
					page_decref(page);
					return pte;
				...
				return (epte_t *) -E_NO_MEM;
		} else if (!((physaddr_t)pdpe & PTE_P) && create == 0) {
			return (epte_t *) -E_NO_ENT;
		} else if ((uint64_t)pdpe & PTE_P) {
			return ept_walk_2(KADDR((uintptr_t)((pdpe_t *)PTE_ADDR(pdpe))),va,create, perm);
		}
	}
	return NULL;
...
// walks the second level of the page table adapted from pdpe_walk
epte_t *
ept_walk_2(pdpe_t *dir,const void *va,int create, uint64_t perm){
#line 661 "../kern/pmap.c"
	if (dir){
		pde_t * pdp = (pde_t *)dir[PDPE(va)];
		if(!((physaddr_t)pdp & PTE_P) && create){
			struct PageInfo *page = NULL;
			if ((page = page_alloc(ALLOC_ZERO))) {
				page->pp_ref += 1;
				dir [PDPE(va)] = page2pa(page)|perm;
				pte_t *pte = ept_walk_3(KADDR((uintptr_t)((pde_t *)PTE_ADDR(dir[PDPE(va)]))),va,create, perm);
				if ((int64_t) pte < 0) return pte;
				else{
					dir[PDPE(va)] = 0;
					page_decref(page);
				...
				return (epte_t *) -E_NO_MEM;
		} else if (!((physaddr_t)pdp & PTE_P) && create == 0) {
			return (epte_t *) -E_NO_ENT;
		} else if((uint64_t)pdp & PTE_P){
			return ept_walk_3(KADDR((uintptr_t)((pde_t *)PTE_ADDR(pdp))),va,create, perm);
		}
	}
	return NULL;
...
// walks the third level of the page table adapted from pgdir_walk
epte_t *
ept_walk_3(pde_t *dir, const void *va, int create, uint64_t perm)
{
#line 695 "../kern/pmap.c"
	if (dir) {
		pte_t *pte  = (pte_t *)dir [PDX(va)];
		if (!((physaddr_t)pte & PTE_P) && create) {
			struct PageInfo *page = NULL;
			if ((page = page_alloc(ALLOC_ZERO))) {
				page->pp_ref += 1;
				dir [PDX(va)] = page2pa(page)|perm;
				return KADDR((uintptr_t)((pte_t *)(PTE_ADDR(dir [PDX(va)])) + PTX(va)));
			}else{
				return (epte_t *) -E_NO_MEM;
			}
		} else if (!((physaddr_t)pte & PTE_P) && create == 0) {
			return (epte_t *)  -E_NO_ENT;
...
int ept_map_hva2gpa(epte_t* eptrt, void* hva, void* gpa, int perm,
    ...
    /* Your code here */
	epte_t* out;
	int err = ept_lookup_gpa(eptrt, gpa, 1, &out);
	if (err != 0) {
		return err;
	}

	if (*out && overwrite == 0 ) return -E_INVAL;
	
	*out = (uint64_t) PADDR(hva) | perm | __EPTE_TYPE( EPTE_TYPE_WB ) | __EPTE_IPAT;
